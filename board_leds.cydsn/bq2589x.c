...
{
    uint8_t value8;
    int result = i2c_read(BQ2589X_ADDR, reg, &value8);
    *value = (int)value8;
    return result;
...
static int bq2589x_write(int reg, int value)
{
    ...
    int result = i2c_write(BQ2589X_ADDR, reg, value8);
    return result;
...
void bq2589x_init(void)
{
    ...
    if (bq2589x_write(BQ2589X_REG_CHG_VOLT, 0x5A)) //4.192 charge voltage
		return;
	if (bq2589x_write(BQ2589X_REG_INPUT_CURR, 0x3F)) // 3.25A?
		return;
	if (bq2589x_write(BQ2589X_REG_CFG1, 0x21)) // ICD ILIM???
		return;
...
void bq2589x_get_status(void)
{
    ...
    batt_mv = 2304 + (batt_mv & 0x7f) * 20;
    sys_mv =  2304 + sys_mv * 20;
    vbus_mv = 2600 + (vbus_mv & 0x7f) * 100;
    input_ma = 100 + (input_ma & 0x3f) * 50;
